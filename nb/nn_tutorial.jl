...
- To build a machine learning model we will use the Flux.jl framework 

- Recognizing handwritten numbers is a supervised machine learning task of image classification. To implement it, we need to have a labeled dataset of handwritten digits and use it to train the machine learning model.
...
# ╔═╡ 15cd0000-59c7-42ee-b74c-12bf4766c86a


# ╔═╡ fd2f89b9-d793-4c53-bccc-599c3b603fc6


# ╔═╡ 6febb6e8-9308-44c0-ac3e-b4d9c5662a2c


# ╔═╡ 1c7faa07-bff2-4c50-9fb5-7f0df81df498
...
# ╔═╡ dca23b34-7bdd-4959-8036-4c4ba40509c7
md"""
## Load images
"""

# ╔═╡ 4832218b-0b75-4677-aad8-a45ce447a9c6
img = load("/Users/jjgomezcadenas/Data/mnist_png/training/0/1.png")

# ╔═╡ 2231ee4c-b502-44a1-b725-f26283d4a218
img2 = load("/Users/jjgomezcadenas/Data/mnist_png/training/0/435.png")

# ╔═╡ 73026a67-24c4-4863-84af-0ade9439990a
size(img)

# ╔═╡ 48d3c472-598a-4a77-a06b-00adec0503c7
datax = Float32.(channelview(img))

# ╔═╡ 988e3447-4972-47e0-b558-de5ea78bdc66
datar = reshape(Float32.(channelview(img)),28,28,1)

# ╔═╡ 61ac7dfc-ec6c-475a-97d5-35e8bfb71774
datar2 = reshape(Float32.(channelview(img2)),28,28,1)

# ╔═╡ 6f7a5d79-358a-4719-84c8-4a7573506b26
X = []

# ╔═╡ 13831605-058d-4578-826d-851b692ed0e4
cat(datar, datar2, dims=3)

# ╔═╡ 81cbb2ca-2e81-4a94-b74a-84bd4790d014
md"""
## Reading the dataset
"""

# ╔═╡ d7abae20-c298-46aa-b05b-30ade42c4161
tpath="/Users/jjgomezcadenas/data/mnist_png/training/"

# ╔═╡ d48e2938-a4cd-4eff-a5db-6d921e64436d
xpath="/Users/jjgomezcadenas/data/mnist_png/testing/"

# ╔═╡ 14e3997a-ba49-470a-a479-451acc591186
function createDataset(path, maxf)
    X = []
    y = []
...
# ╔═╡ f4e6cff6-386d-4731-908a-f9485044fd47
x_train, y_train = createDataset(tpath, 1000)

# ╔═╡ 10a63380-0e71-4924-a76e-d40c5a4ea0d8
x_test, y_test = createDataset(xpath, 1000)

# ╔═╡ 25594853-2682-4853-8a99-2ba99cc23f8e
md"""
## Defining the NN
"""
...
# ╔═╡ 06a6fb62-547a-41e5-9d2f-0572e580aedd
model = Chain(
    Flux.flatten,
    Dense(784=>15,relu),
    Dense(15=>10,sigmoid),
...
# ╔═╡ acab5194-6381-4622-9cd2-729ba2052c7f
md"""
...
# ╔═╡ 10ded94f-ed57-455d-8f24-704d035181e1
predict = model(Flux.unsqueeze(x_train[:,:,1],dims=3))

# ╔═╡ 94e25092-d9dc-4e23-af89-eaf965d1785f
md"""
...
md"""- As you can see, the highest probability has a neuron number 4 (0.11353869) which means that the model predicted the digit "4". However, if you check the real answer in the labels vector:"""

# ╔═╡ edaa1e6a-7967-4f55-8ada-815732cd547f
y_train[1]

# ╔═╡ c979aabb-0dfa-431e-b742-2d1415dbc74e
md"""
...
# ╔═╡ 38245aa7-9299-4c1a-8234-a5097bf2806d
md"""
...
- The function compares these probabilities with the true values from the labels vector and calculates the amount of error.
...
# ╔═╡ 3487e2da-ce7b-44f2-b29d-6b80f597c09f
md"Assemble the training data"


# ╔═╡ 4262bef7-50e3-4e42-b1a4-2c2ad49bb0d1
data = Flux.DataLoader((x_train,y_train), shuffle=true)

# ╔═╡ f5790cef-964e-4328-889a-68512ea90556
md"Initialize the ADAM optimizer with default settings"

# ╔═╡ 3d3aa401-24f8-4ee0-9c0c-f5eb9c5c87bf

optimizer = Flux.setup(Adam(), model)

# ╔═╡ 440893bb-5d8f-4b87-a9ed-eb9a1fe09198
md"""- Define the loss function that uses the cross-entropy to  measure the error by comparing model predictions of data 
...
# ╔═╡ f75d8197-4b6e-4088-9955-b7423709e76a
md"Train the model 10 times in a loop"

# ╔═╡ 2cf72502-e57b-4887-a14d-d44cb99ac686

#for epoch in 1:10
#end
#    Flux.train!(loss, model, data, optimizer)

# ╔═╡ 6b827104-75d6-4fce-ab60-0eda14c4c623
md"""
...
#predict2 = model(Flux.unsqueeze(x_train[:,:,1],dims=3))

# ╔═╡ 6208c4cd-a728-4575-9c6b-17dfe7f22908
md"""
...
# ╔═╡ 2610ba71-cc59-4877-a6eb-5d3dbef5c263
function accuracy()
    correct = 0
...
# ╔═╡ 0685055a-9cc5-41bb-a1c8-1155fe27655e
for epoch in 1:5
    Flux.train!(loss, model, data, optimizer)
    println(accuracy())
end

# ╔═╡ 8d8b292e-0b79-43d9-bfd5-65d608e4f012
md"""
- It shows that accuracy was going up until the 4th iteration. Since then, it started to go down, which could be a sign that the model started to overfit.
...
# ╔═╡ 38b9cb25-b8c8-40f9-9123-f2ec2bdd8ad7
md"""
...
# ╔═╡ 74b7a1d7-054e-439d-a7a4-24277d7727d4
md"""
...
a- ctivation_function is the function that will be applied to each feature after convolution and before sending to the next layer of the network, the same as we did before for Dense layers.

For example, if you add the following Conv layer on top of the others to the Chain:
"""

# ╔═╡ 999c788c-2110-4390-bd6e-c966a38b54c9
load("kernel.png")

# ╔═╡ e5dbbdc5-4e10-44a6-b6fd-a18ef2f40c8d
md"""
```
model = Chain(
    Conv((5,5),1=>6,relu),
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "9c5212e20b1e56f16147c20a986e278e9b1e682f"
