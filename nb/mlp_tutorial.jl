...
# ╔═╡ e045c81a-8c40-11ee-2395-45f2fb7c131f
begin
...
# ╔═╡ 274e7f32-7e94-492b-b13f-edf72202d74c
PlutoUI.TableOfContents(title="SMP", indent=true)

# ╔═╡ 7cf78c62-f6fc-429b-981a-af66754d0d8f
md"""
We set default values for learning rate, batch size, epochs, and the usage of a GPU (if available) for our model:
"""

# ╔═╡ ec8a8b8d-4616-4311-b826-eebad14c4a64
...
# ╔═╡ 8d76e9d3-9f89-417d-be99-5cda6376eee4
md"""
...
# ╔═╡ 0fdd8f32-cd27-4e1a-9c23-0c7257984405
load("nn.png")

# ╔═╡ 03a416b6-c602-4da7-9a23-fe31aa66251e
md"""
In this picture, the input layer contains 784 neurons that should receive the features of each image. The training dataset consists of 28x28 images, which is 784 pixels. This is how this neural network works:
...
# ╔═╡ 8f7e173a-0c9d-43b4-9368-b03deb444a09
load("nn2.png")

# ╔═╡ 54b3812c-7cfd-4718-8102-908e7bacdf58
md"""
## The MNIST data base

The data base was set by the National Institute of Standards and Technology. 
It is split into testing and training data in appropriate folders. Each of these folders contains images of handwritten digits, classified to folders from "0" to "9". There are 60000 images in the training folder and 10000 images in the testing folder.
To load it we use package MLDatasets.
"""

# ╔═╡ c1f0975d-cd72-4075-8904-53a2bf7ad54d
ENV["DATADEPS_ALWAYS_ACCEPT"] = "true" # avoids interative dialog

# ╔═╡ 11285d6b-a8df-44b1-b9d8-ef5ace846af6
md"""
- Load the training data and the test data
"""

# ╔═╡ 3992e0cf-f9b2-481a-89cd-d6c880df98d9
dataset = MNIST(:train)

# ╔═╡ 662a9641-2da4-4e22-a533-f2680e5d77af
tdataset = MNIST(:test)

# ╔═╡ 9ef547bf-e51c-4c7c-a9a1-521ee565fb28
dataset.features[:,:,1]

# ╔═╡ 753673bf-7c22-4868-a12c-cfecb4cd8e56
dataset.targets[1] 

# ╔═╡ 05d5cfb9-7f22-4787-99b5-17fda73622e6
Gray.(dataset.features[:,:,1]) |> transpose

# ╔═╡ 58cb9c2c-0ba1-404b-a557-77e62e2c48e8
md"""
- Get the matrices and labels for the test and traing set
"""

# ╔═╡ 331da40f-f0d5-4b03-b5bc-f6f815e3e1c7
x_test, y_test = tdataset[:];

# ╔═╡ ddd6897e-c06a-4c7c-8af7-1419daed6aea
x_train, y_train = dataset[:];

# ╔═╡ 3be437b9-82b0-4b81-bd81-d591c7d1a292
md"""
- Reshape Data in order to flatten each image into a linear array

"""

# ╔═╡ cc258f7b-505e-4db2-afe2-5987e6c571e7
xtrain = Flux.flatten(x_train);

# ╔═╡ ed08ac13-8398-4be5-9e68-50aea5375085
xtest = Flux.flatten(x_test);

# ╔═╡ a6adfd4c-af09-435a-9970-f771529d8ffa
md"""
- We know use One Hot Encoding to encode the categorigal label data. 

- One hot encoding is a technique that we use to represent categorical variables as numerical values in a machine learning model.
- OHE replaces the categorial variable by a vector whose dimension span the size of the categorial variables set. In this vector, a unique position defines de variable. For example, consider y_train
"""

# ╔═╡ 5e473a76-d8c6-45d5-916d-6edaf095ba8c
y_train

# ╔═╡ f80fa21f-a673-455d-968a-42a7b8cde1ab
md"""
- Notice that the labels go from 0 to 9, corresponding to the 10 digits we want to recognise. When we use one hot encoding, each number is replaces by a vector, such that we find 1 in the category of the label (e.g, 5 gives a 1 in index 6, while 0 gives a 1 in index 1) and zero in the rest. 
"""

# ╔═╡ 8f14e4fe-b104-4578-ba16-d8ce6807e631
ytrain = onehotbatch(y_train, 0:9);

# ╔═╡ 7846f563-661a-4c25-bf50-1e6bd7f7da37
ytrain

# ╔═╡ 1315e0fa-4946-4cf0-9cd4-ef94d50a3789
ytest = onehotbatch(y_test, 0:9);

# ╔═╡ d299d9c9-6e1b-4c99-9651-6442ed6831aa
md"""
...
- We know use the DataLoader to lad data in batches. The DataLoader is an object that iterates over mini-batches of data, each mini-batch containing batchsize observations (except possibly the last one).
...
# ╔═╡ cd96c099-b07b-49e1-8bd0-48115a5af300
Args()

# ╔═╡ c0191552-9209-4e92-89ac-b80bbb2bffc9
train_datax = DataLoader((xtrain, ytrain), batchsize=Args().batchsize, shuffle=true)

# ╔═╡ eb7aaede-73b1-4de8-b5e2-3b22b5e0a510
test_datax = DataLoader((xtest, ytest), batchsize=Args().batchsize);

# ╔═╡ 9742ff15-754b-451e-acd9-6121b52e9b6c
md"""

- We can collect all the above in one function:
...
# ╔═╡ 762544fb-480f-4dbc-b8fd-9eb5f5ef02eb
function getdata(args)
    ...
    ytrain, ytest = onehotbatch(y_train, 0:9), onehotbatch(y_test, 0:9)
...
# ╔═╡ 7407c10e-3a54-4e7a-bd6e-259bab797dfc
md"""
## Model
- A MLP consist of three layers that are fully connected. For this example, we define out model with the following layers and dimensions:
...
To build the model with use the ```Dense``` function:
```
Dense(in, out, σ=identity; bias=true, init=glorot_uniform)
```

- which takes the dimension of the input (in) and output (out) layers, together with an activation function. The first layer is then defined with:
```
Dense(prod(imgsize), hidden, relu)
```
- where prod(imgsize)= 28 x 28 x 1, hidden=32 and we use relu as activation. 

- We then chain to the following layer
```
Dense(hidden, nclasses))
```

- which goes from the hidden layer (32 neurons) to the output layer (10 classes)
...
# ╔═╡ 9b63ad13-b096-430d-bec5-2ca1a97206ee
function build_model(; imgsize=(28,28,1), hidden=32, nclasses=10)
    return Chain(
 	    Dense(prod(imgsize), hidden, relu),
            Dense(hidden, nclasses))
end

# ╔═╡ 8ee9506f-c436-4f40-819f-0d36f8391b77
md"""
## Loss functions
...
# ╔═╡ 0d4dd2ed-f49c-4e1c-aff0-f682d1cf81aa
...
# ╔═╡ 98e40369-b599-4ca4-8e09-35bffdb9aed4
md"""
- In addition, we define the function ```accuracy``` to report the accuracy of our model during the training process. To compute the accuray, we need to decode the output of our model using the onecold function, which is an inverse of onehot.

- The function compares the output of the model (```model(x)```) with the value of the label. If both are the same, it adds one unit. Then sums and normalises. 
"""

# ╔═╡ f58c778c-5c4a-4fa8-8d31-fb26a7c383c6
...
# ╔═╡ 0efee762-8727-4bae-a7d0-6e034f51feba
md"""
## Train the model
"""

# ╔═╡ 700cfc5d-1838-4979-a5d5-17708c107416
md"""
- Initializing Model parameters 
"""

# ╔═╡ 40e85df3-2354-4a2d-a32e-c759e51eca34
args = Args()

# ╔═╡ a33c5669-af95-486d-ad35-62e9656f2281
md"- Load Data"
   

# ╔═╡ 3e39c1fc-919f-4507-87cc-1201e3787b44
 train_data,test_data = getdata(args)

# ╔═╡ 004e1e7f-7586-4e43-b90c-58245596ff2e
md"""
- Construct model
"""

# ╔═╡ 3d4e349f-3588-4a7f-a20d-44cab18bad69
m = build_model()

# ╔═╡ eda24703-df29-40a7-b0f3-7c211098bac3
md" - loss function "

# ╔═╡ cbd2a9d0-e881-4ea6-a474-dcc4e9d2e69f
loss(x,y) = logitcrossentropy(m(x), y)

# ╔═╡ 31ee8e02-0d8c-4153-8e30-2224f631d947
md"""

- We use the ```train``` function from Flux.
...
- Initialize the model parameters: Create the args object that contains the defult values for training our model.

- Load the train and test data: Call the function getdata we defined above.

- Construct the model: Build the model and loads the train and test data sets.

- Train the model: Define the callback function evalcb to show the value of the loss_all function during the training process. Then, it sets [Adam](@ref Flux.Optimise.Adam) as the optimiser for training out model. Finally, it runs the training process for 10 epochs (as defined in the args object) and shows the accuracy value for the train and test data.
"""


# ╔═╡ 5199febc-ded4-4c02-9dc2-cca211f3e432
begin
...
# ╔═╡ fc838677-8b93-4359-af3f-56a5f12f3c0f
for epoch in 1:args.epochs
        @info "Epoch $epoch"
        Flux.train!(loss, params(m), train_data, opt, cb = evalcb)
end

# ╔═╡ 6416a5f2-f21b-4d74-b717-5c05f1b39bbf
@show accuracy(train_data, m)

# ╔═╡ e82a16d9-a399-4c82-8129-1896094b419b
@show accuracy(test_data, m)

# ╔═╡ ac4ef016-7f7a-46f9-b0e6-5b739ffe2a1a
md"""
## Inspect the data
"""

# ╔═╡ 24beec79-2276-492c-a06f-843ef4d6b4fb
md" choose image (image number) $(@bind in NumberField(0:60000; default=1))"

# ╔═╡ 47dae01c-8add-4ca1-b8aa-f3301433c2f9
xtd = train_data.data[1];

# ╔═╡ 8fa7c079-bd1a-4964-941b-0558c8ba9764
reshape(xtd[:,in], 28, 28);

# ╔═╡ 87ceefd5-388d-491f-b1ce-ea2094d60256
reshape(xtd[:,in], 28, 28) .|> Gray |> transpose

# ╔═╡ 14fada23-3733-44a7-8219-21b02f25838e
ytd = train_data.data[2];

# ╔═╡ b2d31278-8403-4cb1-9e2e-3d37370db376
md"""
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "ec60dd2a6a2bd883c913dfdc354a335e5d1f70e9"
